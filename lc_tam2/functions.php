    ...
    for ($x = 1; $x <= $row["COUNT(dept_id)"]; $x++)
    {
        ...
        if($row3['COUNT(course_id)'] != 0)
        {
            ...
            for ($y = 1; $y <= $row3['COUNT(course_id)']; $y++)
            {
                ...
                echo '<button type="submit" form="form" formmethod="POST" formaction="select_professor.php" class="btn btn-success display-4" name="' . $dept . '_course_' . $y . '" value="' . $row4["course_id"] . '">' . $row4["course_id"] . '</button>';
            }
        
            echo '
                                        <form id="form">
                                            <input type="hidden" name="course_ready">
                                        </form>
                                    </div>
                                </p>
                            </div>
    ...
    if(isset($_POST["course_ready"]))
    {
        ...
        do
        {
            ...
            if(isset($_POST["course_{$i}"]))
                $_SESSION['selected_course'] = $_POST["course_{$i}"];
            
        }while(!isset($_POST["course_{$i}"]));
...
{
    $query = query("SELECT professor_entry_id, professor_name, professor_initial, professor_first_lastname, professor_second_lastname 
    FROM lc_professors WHERE course_id = '" . $_SESSION['selected_course'] . "'");
    confirm($query);
                ...
                <div class="card-wrap">
                    ...
                    <div class="content-wrap">
                        ...
                        <div class="mbr-section-btn card-btn align-center">
                            <button type="submit" form="form" formmethod="POST" formaction="select_tutor.php" class="btn btn-primary display-4" name="professor_' . $x . ' value="' . $row["professor_entry_id"] . '">Select</button>
                        </div>
                    </div>
                </div>
            </div>';
...
{
    $_SESSION['selected_course'] = $_POST["professor_{$i}"];
    
    $query = query("SELECT student_name, student_initial, student_first_lastname, student_second_lastname FROM lc_students WHERE cosurse_id = '" . $_SESSION['selected_course'] . "'");
    confirm($query);
...
function view_student_history_appoints()
{
    $query5 = query("SELECT  FROM  WHERE ");
    confirm($query5);
